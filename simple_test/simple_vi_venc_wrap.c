...
static int vi_chn_init(int channelId, int width, int height, int maxWidth, int maxHeight) {
    int ret;
    int buf_cnt = 3;
    // VI init
    ...
    g_vi_chn_attr.stIspOpt.enMemoryType = VI_V4L2_MEMORY_TYPE_DMABUF;//VI_V4L2_MEMORY_TYPE_MMAP;
    ...
    if (g_bWrapEn) {
        ...
        printf("set channel wrap line: %d, wrapBuffSize = %d", g_u32WrapLine, g_stViWrap.u32WrapBufferSize);
...
static void init_venc_cfg(RK_U32 u32Ch, RK_CODEC_ID_E enType) {
    ...
    g_venc_cfgs[u32Ch].stAttr.stVencAttr.u32BufSize = g_u32MaxWidth * g_u32MaxHeight * 3 / 2;
    ...
    while (!quit) {
        ...
        if (s32Ret == RK_SUCCESS) {
            ...
            printf("chn:0, loopCount:%d enc->seq:%d wd:%d pts=%lld delay=%lldus\n", loopCount,
                        g_venc_stFrame[0].u32Seq, g_venc_stFrame[0].pstPack->u32Len,
                        g_venc_stFrame[0].pstPack->u64PTS,
                        nowUs - g_venc_stFrame[0].pstPack->u64PTS);

            s32Ret = RK_MPI_VENC_ReleaseStream(g_venc_cfgs[0].s32ChnId, &g_venc_stFrame[0]);
            if (s32Ret != RK_SUCCESS) {
...
int main(int argc, char *argv[])
{
    ...
    snprintf(stDebugFile.aFileName, sizeof(stDebugFile.aFileName), "venc_wrap_%d.bin",s32chnlId);
