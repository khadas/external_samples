...
int vi_chn_init(int channelId, int width, int height) {
	...
	vi_chn_attr.stIspOpt.enMemoryType = VI_V4L2_MEMORY_TYPE_MMAP;//VI_V4L2_MEMORY_TYPE_DMABUF;//VI_V4L2_MEMORY_TYPE_MMAP;
...
void FaceAnalyseResultCallback(const RockIvaFaceCapResult* result, const RockIvaExecuteStatus status, void* userData)
{
	...
	if (status == ROCKIVA_SUCCESS) {
		...
		snprintf(save_path, PATH_MAX, "%s/%d_%d_%d_%dx%d_mask%d_face.yuv", OUT_CAPTURE_PATH, result->faceInfo.objId, result->faceInfo.faceQuality.score,
